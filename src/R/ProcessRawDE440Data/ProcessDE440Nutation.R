# Extract data for Nutation from the DE440 ephemeris and store the data
# in parquet file format. Pass the file name, block size, and file length

ProcessDE440Nutation <- function(filename, fileBlocks, fileLength)
{
  ...
  numColumnsNutation = DE440NUMCOEFFNUTATION*2+3
  numRowsNutation = fileBlocks * DE440NUMSUBINTNUTATION
  ...
  for (i in seq(from = 1, to = numRowsNutation, by = DE440NUMSUBINTNUTATION)){
  ...
  k <- 836
  for (i in seq(from = 2, to = numRowsNutation, by = DE440NUMSUBINTNUTATION)){
    for (j in seq(from = 3, to = 22, by = 1)){
      nutation_data[i, j+1] <- vect[j+k]
    }
    k <- k + 1020
  ...
  k <- 856
  for (i in seq(from = 3, to = numRowsNutation, by = DE440NUMSUBINTNUTATION)){
    for (j in seq(from = 3, to = 22, by = 1)){
      nutation_data[i, j+1] <- vect[j+k]
    }
    k <- k + 1020
  ...
  k <- 876
  for (i in seq(from = 4, to = numRowsNutation, by = DE440NUMSUBINTNUTATION)){
    for (j in seq(from = 3, to = 22, by = 1)){
      nutation_data[i, j+1] <- vect[j+k]
    }
    k <- k + 1020
  ...
  fnn <- paste(sep = "", "Nutation_", fn, "_440", ".parquet")
