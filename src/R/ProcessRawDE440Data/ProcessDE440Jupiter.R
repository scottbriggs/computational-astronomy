# Extract data for Jupiter from the DE440 ephemeris and store the data
# in parquet file format. Pass the file name, block size, and file length

ProcessDE440Jupiter <- function(filename, fileBlocks, fileLength)
{
  log_info('Process data for Jupiter')
  ...
  numColumnsJupiter = DE440NUMCOEFFJUPITER*3+3
  numRowsJupiter = fileBlocks * DE440NUMSUBINTJUPITER
  jupiter_col_names <- c("Julian_Day_Start", "Julian_Day_End", "INTERVAL",
                         "X1", "X2", "X3", "X4", "X5", "X6", "X7", "X8",
                         "Y1", "Y2", "Y3", "Y4", "Y5", "Y6", "Y7", "Y8",
                         "Z1", "Z2", "Z3", "Z4", "Z5", "Z6", "Z7", "Z8")
  jupiter_data = matrix(0.0,nrow=numRowsJupiter,ncol=numColumnsJupiter)
  colnames(jupiter_data) <- jupiter_col_names
  
  # Populate the intervals for Jupiter
  for (i in seq(from = 1, to = numRowsJupiter, by = DE440NUMSUBINTJUPITER)){
  ...
  fnn <- paste(sep = "", "Jupiter_", fn, "_440", ".parquet")
