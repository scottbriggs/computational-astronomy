# Extract data for Venus from the DE440 ephemeris and store the data
# in parquet file format. Pass the file name, block size, and file length

ProcessDE440Venus <- function(filename, fileBlocks, fileLength)
{
  ...
  numColumnsVenus = DE440NUMCOEFFVENUS*3+3
  numRowsVenus = fileBlocks * DE440NUMSUBINTVENUS
  ...
  for (i in seq(from = 1, to = numRowsVenus, by = DE440NUMSUBINTVENUS)){
  ...
  k <- 198L
  for (i in seq(from = 2, to = numRowsVenus, by = DE440NUMSUBINTVENUS)){
    for (j in seq(from = 3, to = 32, by = 1)){
      venus_data[i, j+1] <- vect[j+k]
    }
    k <- k + 1020
  ...
  fnn <- paste(sep = "", "Venus_", fn, "_de440", ".parquet")
