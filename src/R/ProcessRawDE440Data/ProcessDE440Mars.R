# Extract data for Mars from the DE440 ephemeris and store the data
# in parquet file format. Pass the file name, block size, and file length

ProcessDE440Mars <- function(filename, fileBlocks, fileLength)
{
  ...
  numColumnsMars = DE440NUMCOEFFMARS*3+3
  numRowsMars = fileBlocks * DE440NUMSUBINTMARS
  ...
  fnn <- paste(sep = "", "Mars_", fn, "_440", ".parquet")
