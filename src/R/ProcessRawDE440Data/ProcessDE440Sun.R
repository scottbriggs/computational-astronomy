# Extract data for the Sun from the DE440 ephemeris and store the data
# in parquet file format. Pass the file name, block size, and file length

ProcessDE440Sun <- function(filename, fileBlocks, fileLength)
{
  ...
  numColumnsSun = DE440NUMCOEFFSUN*3+3
  numRowsSun = fileBlocks * DE440NUMSUBINTSUN
  ...
  for (i in seq(from = 1, to = numRowsSun, by = DE440NUMSUBINTSUN)){
  ...
  k <- 783
  for (i in seq(from = 2, to = numRowsSun, by = DE440NUMSUBINTSUN)){
    for (j in seq(from = 3, to = 35, by = 1)){
      sun_data[i, j+1] <- vect[j+k]
    }
    k <- k + 1020
  ...
  fnn <- paste(sep = "", "Sun_", fn, "_440", ".parquet")
