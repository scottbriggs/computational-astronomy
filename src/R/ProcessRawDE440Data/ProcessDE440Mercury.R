# Extract data for Mercury from the DE440 ephemeris and store the data
# in parquet file format. Pass the file name, block size, and file length

ProcessDE440Mercury <- function(filename, fileBlocks, fileLength)
{
  ...
  numColumnsMercury = DE440NUMCOEFFMERCURY*3+3
  numRowsMercury = fileBlocks * DE440NUMSUBINTMERCURY
  ...
  for (i in seq(from = 1, to = numRowsMercury, by = DE440NUMSUBINTMERCURY)){
  ...
  k <- 42
  for (i in seq(from = 2, to = numRowsMercury, by = DE440NUMSUBINTMERCURY)){
    for (j in seq(from = 3, to = 44, by = 1)){
      mercury_data[i, j+1] <- vect[j+k]
    }
    k <- k + 1020
  ...
  k <- 84
  for (i in seq(from = 3, to = numRowsMercury, by = DE440NUMSUBINTMERCURY)){
    for (j in seq(from = 3, to = 44, by = 1)){
      mercury_data[i, j+1] <- vect[j+k]
    }
    k <- k + 1020
  ...
  k <- 126
  for (i in seq(from = 4, to = numRowsMercury, by = DE440NUMSUBINTMERCURY)){
    for (j in seq(from = 3, to = 44, by = 1)){
      mercury_data[i, j+1] <- vect[j+k]
    }
    k <- k + 1020
  ...
  fnn <- paste(sep = "", "Mercury_", fn, "_440", ".parquet")
