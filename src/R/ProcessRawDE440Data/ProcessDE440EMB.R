# Extract data for the Earth-Moon Barycenter from the DE440 ephemeris and
# store the data in parquet file format.
# Pass the file name, block size, and file length

ProcessDE440EMB <- function(filename, fileBlocks, fileLength)
{
  ...
  numColumnsEMB = DE440NUMCOEFFEMB*3+3
  numRowsEMB = fileBlocks * DE440NUMSUBINTEMB
  ...
  for (i in seq(from = 1, to = numRowsEMB, by = DE440NUMSUBINTEMB)){
  ...
  k <- 267
  for (i in seq(from = 2, to = numRowsEMB, by = DE440NUMSUBINTEMB)){
    for (j in seq(from = 3, to = 41, by = 1)){
      emb_data[i, j+1] <- vect[j+k]
    }
    k <- k + 1020
  ...
  fnn <- paste(sep = "", "EMB_", fn, "_440", ".parquet")
