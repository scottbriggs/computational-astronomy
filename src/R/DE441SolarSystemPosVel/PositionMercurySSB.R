# Calculate the position of Mercury using the JPL DE 441 Ephemeris data
# based on a julian day number referencing the solar system barycenter

PositionMercurySSB <- function(jd)
{
  ...
  df_mercury <- df_jd[df_jd$INTERVAL == subinterval,]
  df_mercury <- subset(df_mercury, 
  ...
  chebyshev <- data.frame(matrix(0.0, nrow=DE441NUMCOEFFMERCURY, ncol=2))
  chebyshev[1,1] <- 1.0
  chebyshev[2,1] <- x
  chebyshev[1,2] <- 0.0
  chebyshev[2,2] <- 1.0
  ...
  for (i in seq(from = 3, to = DE441NUMCOEFFMERCURY, by = 1)){
  ...
  v <- 0
  for (v in seq(from = DE441NUMCOEFFMERCURY, to = 1, by = -1)){
    pos_vel[1,1] <- pos_vel[1,1] + (chebyshev[v,1] * df_mercury[1,v])
    pos_vel[2,1] <- pos_vel[2,1] + (chebyshev[v,1] * df_mercury[1,v+DE441NUMCOEFFMERCURY])
    pos_vel[3,1] <- pos_vel[3,1] + (chebyshev[v,1] * df_mercury[1,v+2*DE441NUMCOEFFMERCURY])
    
    pos_vel[1,2] <- pos_vel[1,2] + (chebyshev[v,2] * df_mercury[1,v])
    pos_vel[2,2] <- pos_vel[2,2] + (chebyshev[v,2] * df_mercury[1,v+DE441NUMCOEFFMERCURY])
    pos_vel[3,2] <- pos_vel[3,2] + (chebyshev[v,2] * df_mercury[1,v+2*DE441NUMCOEFFMERCURY])
  }
  
  # Scale the velocity
  scale_value <- 2 * DE441NUMSUBINTMERCURY / DE441DAYSPERBLOCK
