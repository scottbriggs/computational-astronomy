# Calculate the position of Uranus using the JPL DE 441 Ephemeris data
# based on a julian day number referencing the solar system barycenter

PositionUranusSSB <- function(jd)
{
  ...
  df_uranus <- df_jd[df_jd$INTERVAL == subinterval,]
  df_uranus <- subset(df_uranus, 
  ...
  chebyshev <- data.frame(matrix(0.0, nrow=DE441NUMCOEFFURANUS, ncol=2))
  chebyshev[1,1] <- 1.0
  chebyshev[2,1] <- x
  chebyshev[1,2] <- 0.0
  chebyshev[2,2] <- 1.0
  ...
  for (i in seq(from = 3, to = DE441NUMCOEFFURANUS, by = 1)){
  ...
  v <- 0
  for (v in seq(from = DE441NUMCOEFFURANUS, to = 1, by = -1)){
    pos_vel[1,1] <- pos_vel[1,1] + (chebyshev[v,1] * df_uranus[1,v])
    pos_vel[2,1] <- pos_vel[2,1] + (chebyshev[v,1] * df_uranus[1,v+DE441NUMCOEFFURANUS])
    pos_vel[3,1] <- pos_vel[3,1] + (chebyshev[v,1] * df_uranus[1,v+2*DE441NUMCOEFFURANUS])
    
    pos_vel[1,2] <- pos_vel[1,2] + (chebyshev[v,2] * df_uranus[1,v])
    pos_vel[2,2] <- pos_vel[2,2] + (chebyshev[v,2] * df_uranus[1,v+DE441NUMCOEFFURANUS])
    pos_vel[3,2] <- pos_vel[3,2] + (chebyshev[v,2] * df_uranus[1,v+2*DE441NUMCOEFFURANUS])
  }
  
  # Scale the velocity
  scale_value <- 2 * DE441NUMSUBINTURANUS / DE441DAYSPERBLOCK
