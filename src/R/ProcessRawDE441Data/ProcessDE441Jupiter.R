# Extract data for Jupiter from the DE441 ephemeris and store the data
# in parquet file format. Pass the file name, block size, and file length

ProcessDE441Jupiter <- function(filename, fileBlocks, fileLength)
{
  ...
  numColumnsJupiter = DE441NUMCOEFFJUPITER*3+3
  numRowsJupiter = fileBlocks * DE441NUMSUBINTJUPITER
  ...
  for (i in seq(from = 1, to = numRowsJupiter, by = DE441NUMSUBINTJUPITER)){
