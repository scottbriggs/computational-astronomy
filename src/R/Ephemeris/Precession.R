...
Obliquity <- function(jd)
{
  ...
  epsilonA_C = c(753.872780, -247.805823, 379.471484, -53.880558, -90.109153,
                 -353.600190, -63.115353, -28.248187, 17.703387, 38.911307)
  
  epsilonA_S = c(-1704.720302, -862.308358, 447.832178, -889.571909,
                 190.402846, -56.564991, -296.222622, -75.859952,
                 67.473503, 3.014055)
  
  P2 <- c(409.90, 396.15, 537.22, 402.90, 417.15, 288.92, 4043.00, 306.00,
          277.00, 203.00)
  
  sum_ep <- 0.0
  for(i in 1:10) {
    sum_ep <- sum_ep + epsilonA_C[i]*cos(PI2*T/P2[i]) + epsilonA_S[i]*sin(PI2*T/P2[i])
  }
  
  ob <- 84028.206305 + (0.3624445 * T) - (0.00004039 * T2) - (110E-9 * T3) + sum_ep
